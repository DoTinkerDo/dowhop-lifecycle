...
var createProfileFormBtn = document.getElementById(
...
function createProfile(e) {
  ...
  profileImageFiles.forEach(function(file, idx) {
    var filePath =
      "userImages/" + uid + "/" + "profileActivityImages/" + file.name;
...
function retrieveProfile(currentProfile) {
  ...
  profileRef.on("value", function(snap) {
    ...
    myProfileEmail.innerHTML =
      "<a href='mailto:" + appUser.email + " '>Send Message</a> ";
    ...
    activityImage1.src =
      (appUser.profileActivityImageURLs &&
        appUser.profileActivityImageURLs.image1) ||
      "/images/placeholder-image1.png";
    activityImage2.src =
      (appUser.profileActivityImageURLs &&
        appUser.profileActivityImageURLs.image2) ||
      "/images/placeholder-image2.png";
    activityImage3.src =
      (appUser.profileActivityImageURLs &&
        appUser.profileActivityImageURLs.image3) ||
...
function socialMediaTW() {
  currentProfile =
    retrieveUrl(window.location.href) || firebase.auth().currentUser.uid;
  var profileRef = firebase.database().ref("app_users/" + currentProfile);
  profileRef.on("value", function(snap) {
    console.log(currentProfile);
